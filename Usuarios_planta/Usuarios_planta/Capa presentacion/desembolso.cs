...
namespace Usuarios_planta
{
    public partial class desembolso : Form
    {
        ...
        private void BtnFormalizacion_Click(object sender, EventArgs e)
        {
            if (usuario.Area == "Administrador" || usuario.Area=="Desembolso")
            {
                AbrirFormHijo(new Formalizacion());
            }
            else
            {
                MessageBox.Show("Usuario no tiene permisos para ver este modulo");
            }           
        }

        private void BtnFormalizacion_MouseHover(object sender, EventArgs e)
        {
            ActivateButton(sender, RGBColors.color2);
            labelInicio.Text = BtnFormalizacion.Text;
            icon2.IconChar = BtnFormalizacion.IconChar;
            icon2.IconColor = BtnFormalizacion.IconColor;
        }

        private void BtnAsignacion_Desembolso_Click(object sender, EventArgs e)
        {
            if (usuario.Area=="Administrador" || usuario.Perfil == "Asignador")
            {
                AbrirFormHijo(new Asignacion_desembolso());
            }
            else
            {
                MessageBox.Show("Usuario no permitido para ver este modulo");
            }            
        }

        private void BtnAsignacion_Desembolso_MouseHover(object sender, EventArgs e)
        {
            ActivateButton(sender, RGBColors.color4);
            labelInicio.Text = BtnOrden.Text;
            icon2.IconChar = BtnOrden.IconChar;
